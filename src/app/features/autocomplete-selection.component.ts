...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, EUI_AUTOCOMPLETE],
  styles: [
    `
  ...
  template: `
    ...
    <eui-autocomplete
      #autocomplete
      [autocompleteData]="this.optionsForAutocomplete$ | async"
      [matching]="'contains'"
      [isReadonly]="readonly"
      [hasChips]="false"
...
export class AutocompleteSelectionComponent<T> implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  private initSubs() {
    ...
    this.formControl.valueChanges.pipe(takeUntil(this.destroy$)).subscribe((matchedOption: EuiAutoCompleteItem<T>) => {
      console.log('old form: internal form control value changed: ', matchedOption);
    ...
    this.writeValueEvents$
      .pipe(
        tap((writeValueInput) => console.log('new form: combineLatest fired with value: ', writeValueInput)),
        withLatestFrom(this.optionsForAutocomplete$),
        map(([writeValueInput, options]) => {
          ...
          const valueReset = assignedOptionValue !== writeValueInput;
          return { matchedOption, valueReset };
        }),
        filter(({ matchedOption }) => matchedOption !== this.formControl.value),
    ...
    this.optionsForAutocomplete$.pipe(takeUntil(this.destroy$)).subscribe((options) => {
      ...
      console.log('old form: clearing value as no match found');
  ...
  writeValue(value: T): void {
    console.log('old form: writeValue called with value: ', value);
    if (undefined === value) {
      //return
    }
